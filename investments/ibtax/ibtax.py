...
def prepare_dividends_report(dividends: List[Dividend], exchange_rate_provider: ExchangeRatesProvider, verbose: bool) -> pandas.DataFrame:
    ...
    df_data = [(i + 1, x.ticker, x.ticker.issuer_country_code, pandas.to_datetime(x.date), x.amount, x.tax) for i, x in enumerate(dividends)]
    df = pandas.DataFrame(df_data, columns=['N', 'ticker', 'issuer_country_code', 'date', 'amount', 'tax_paid'])
